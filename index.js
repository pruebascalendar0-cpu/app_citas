...
// Cliente OAuth2 con refresh token
...
/**
 * Construye un mensaje RFC-2822 y lo envía con Gmail API.
 * IMPORTANTE:
 *  - "From" debe ser el EMAIL_USER o un alias verificado.
 *  - Puedes personalizar el "nombre" en el header From.
 */
async function gmailSend({ to, subject, html, fromEmail = EMAIL_USER, fromName = EMAIL_FROM, replyTo = REPLY_TO }) {
  // Encabezado Subject con UTF-8 seguro
  ...
  try {
    ...
    console.log("[GMAIL_API] Enviado OK, id:", res.data.id);
    return res.data;
  } catch (err) {
    console.error("[GMAIL_API] Error al enviar:", err?.response?.data || err.message);
    throw err;
  }
}

// ======= TUS FUNCIONES DE NEGOCIO (reusando gmailSend) =======
