...
async function enviarMail({ to, subject, html, text, category = "notificaciones" }) {
  ...
  try { await sg.send(msg); } catch (err) { if (err.response?.body) console.error("❌ SG Error:", JSON.stringify(err.response.body, null, 2)); else console.error("❌ SG Error:", err); }
}
...
app.post("/usuario/recuperar-correo", (req, res) => {
  ...
  const consulta = `SELECT usuario_correo FROM usuarios
...
app.post("/usuario/reset/cambiar", (req, res) => {
  ...
  conexion.query(q, [usuario_correo], (err, rows) => {
    ...
    conexion.query("UPDATE usuarios SET usuario_contrasena_hash=?, reset_codigo=NULL WHERE id_usuario=?", [hashed, u.id_usuario], (e2) => {
...
app.post("/cita/agregar", (req, res) => {
  ...
  conexion.query(qOrden, [id_usuario], (error, results) => {
    ...
    conexion.query(qInsert, [id_usuario, id_medico, cita_fecha, cita_hora, numero_orden], (e2) => {
      ...
      conexion.query(ocupar, [cita_fecha, cita_hora, id_medico], (e3) => { if (e3) console.warn("No se pudo marcar el horario ocupado:", e3.message); });
  ...
  conexion.query("SELECT usuario_correo FROM usuarios WHERE id_usuario=?", [id_usuario], (e0, rows) => {
    ...
    conexion.query(qAnterior, [id], (e1, r1) => {
      ...
      const qLiberar = `
        ...
        WHERE horario_fecha = STR_TO_DATE(?, '%Y-%m-%d')
